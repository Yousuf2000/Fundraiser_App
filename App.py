...
def main():
    ...
    with col2:
        ...
        if submit_donation:
            if is_valid_phone_number(phone_number) and donation_amount > 0 and payment_type:
                ...
                st.error("Please fill in all the fields correctly before donating.")
    ...
    st.write("To download donor details, please enter the admin password.")
    ...
    if st.button("Submit"):
        if password == ADMIN_PASSWORD:
            ...
            if not st.session_state.donor_data.empty:
                ...
                st.write("No donor details available to download.")
        else:
